"use client";
import { clearCart, removeFromCart } from "@/lib/state/cartSlice";
    ...
    <div className="h-screen w-11/12">
      ...
      {!cart.items.length ? (
        <div className="flex flex-col items-center">
          ...
          <Link
            href="/"
            className="pl-6 pr-6 pt-3 pb-3 mt-6 font-bold bg-orange-500 text-white rounded-full hover:bg-orange-600 transition-colors"
          >
            К ассортименту
          </Link>
        </div>
        ...
        <div className="flex flex-col h-full items-end">
          <ul className="h-1/2 overflow-y-scroll w-full">
            {cart.items.map((item) => (
              <li
                key={item.id}
                className="flex pl-4 pr-4 items-center relative"
              >
                <Image
                  src={item.thumbnail}
                  sizes="100vw"
          ...
          <button
            className="pl-6 pr-6 pt-3 pb-3 mt-6 font-bold bg-orange-500 text-white rounded-full hover:bg-orange-600 transition-colors self-end"
            onClick={handleOrder}
          >
            Заказать
          </button>
